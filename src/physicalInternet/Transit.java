...
public class Transit {
	...
	private static ArrayList<Order> ordersToMainHub = new ArrayList<Order>();
	private static ArrayList<Order> ordersToRegionalHub = new ArrayList<Order>();
	...
	public void transitToReceiver(int time, Receiver receiver) {
		for (Iterator<Order> iterator = ordersToReceiver.iterator(); iterator.hasNext();) {
			...
			if (order.getTimeOfOrder() + transitTime + 1 == time) {
	...
	public void sendOrdersToMainHub(Hub mainHub, Order order) {
		ordersToMainHub.add(order);
		for (Iterator<Order> iterator = ordersToMainHub.iterator(); iterator.hasNext();) {
			Order currentOrder = iterator.next();

			if (currentOrder.getTimeOfOrder() + transitTime + 3 == Simulation.currentTime) {
				sendOrderToHub(order, mainHub);
				iterator.remove();
			}
		}
	...
	public void sendOrdersToRegionalHub(Hub regionalHub, Order order) {
		ordersToMainHub.add(order);
		for (Iterator<Order> iterator = ordersToMainHub.iterator(); iterator.hasNext();) {
			Order currentOrder = iterator.next();

			if (currentOrder.getTimeOfOrder() + transitTime + 4 == Simulation.currentTime) {
				sendOrderToHub(order, regionalHub);
				iterator.remove();
			}
		}
